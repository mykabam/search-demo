...
var searchByAction = {
  courses: function(r) {
    return {
      ...
      start_date: (new Date(r._source.start_date)).toDateString(),
      price: r._source.price,
      categories: r._source.categories.join(", ")
    }
  },
...
es._filteredSearch = function(q, by, filters, callback) {
  ...
  console.log(JSON.stringify(filter_queries));
  ...
  var types = (function() {
    ...
    TYPES.forEach(function(type) {
      _query[type] = [
        ...
        queries[type].search(q, 100, true)
      ];
    });
    return _query;
  })();
...
module.exports = function(app) {
  ...
  app.get("/search", validateSearchBy, filters, search, facets, function(req, res) {
    if (req.query.callback) {
      res.send(req.query.callback + '(' + JSON.stringify(req.results) + ');');
    } else {
