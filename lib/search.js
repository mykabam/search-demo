...
es._filteredSearch = function(q, by, filters, callback) {
  var range_filters = ["price", "start_date"];
  var terms_filters = ["school", "categories"];

  var filter_queries = [];

  Object.keys(filters).map(function(f) {
    // It's a range filter
    if(range_filters.indexOf(f) > -1) {
      var ranges = filters[f].split(",").map(function(r) {
        return JSON.parse(r);
      });

      filter_queries.push(queries[by].filters[f](ranges));
    }
    // It's a terms filter
    if(terms_filters.indexOf(f) > -1) {
      var terms = filters[f].split(",");
      filters_queries.push(queries[by].filters[f](terms));
    }
  })();
...
module.exports = function(app) {
  ...
  function filters(req, res, next) {
    var filters = [
      "courses.price", "courses.start_date", "courses.categories", "courses.school"
    ];
